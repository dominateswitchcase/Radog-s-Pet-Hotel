...
<<<<<<< Updated upstream
    ...
    // Ocular_Exam_Passed, Vetcard_Verified) live exclusively on the BOOKING
    // table in the DB schema.  At registration time there is no booking row
...
>>>>>>> Stashed changes
        $docs_stmt = $pdo->prepare("
            SELECT pd.DOC_ID,
                   ...
                   pd.PET_ID,
                   NVL(dd.VERIFICATION_STATUS, 'Pending') AS VERIFICATION_STATUS,
...
<<<<<<< Updated upstream
                ...
                <!--
                    ...
                    When a pet has an active booking (Confirmed/Pending), saving here
                    updates that booking row.  If no active booking exists the values
...
<<<<<<< Updated upstream
    ...
    petDocs.forEach(function(doc) {
        // Guard: VERIFICATION_STATUS may be null if DOCUMENT_DETAILS row is missing
...
<<<<<<< Updated upstream
            ...
            if (ownerPets.length === 0) {
                ...
                placeholder.value       = '';
                placeholder.textContent = '— No active pets —';
                ...
                document.getElementById('editDocumentsList').innerHTML =
                    '<div class="doc-empty">No active pets registered for this owner.</div>';
                return;
            }
...
<<<<<<< Updated upstream
            ...
            // Auto-select and fill the first pet
            petSelectEl.value = ownerPets[0].PET_ID;
